...
if [ "$HTTP_CODE" = "302" ]; then
  echo "  ✓ Server responded with HTTP $HTTP_CODE (302 = success, Apps Script processed the request)"
elif [ "$HTTP_CODE" = "200" ]; then
  echo "  ✓ Server responded with HTTP $HTTP_CODE (200 = success)"
else
  echo "  ✗ Server responded with HTTP $HTTP_CODE"
  echo "    Expected 302 (success redirect) or 200"
fi
echo ""

echo "[3/4] Manual verification required:"
echo "  Open the spreadsheet and look for office = '$TEST_ID'"
echo "  → $SHEET_URL"
echo ""
echo "  If the entry is there: ✅ Deployment is working correctly"
echo "  If not, wait 30 seconds and refresh the sheet"
echo ""
echo "  Remember to delete the test row afterwards."
